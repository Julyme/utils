...
//          stateData.getCurrentMasterKey(), stateData.getNextMasterKey());
    ...
    for (String childNodeName : childNodes) {
      ...
      try {
        if (childNodeName.startsWith(DELEGATION_KEY_PREFIX)) {
          ...
          key.readFields(fsIn);
          rmState.getRMDTSecretManagerState().getMasterKeyState().add(key);
  ...
  private void loadRMSequentialNumberState(RMState rmState) throws Exception {
    ...
    if (seqData != null) {
      ...
      try {
//        rmState.getrmrmSecretManagerState.dtSequenceNumber = seqIn.readInt();
      } finally {
        seqIn.close();
      }
    }
    ...
    for (String childNodeName : childNodes) {
      ...
      try {
        if (childNodeName.startsWith(DELEGATION_TOKEN_PREFIX)) {
          ...
          rmState.getRMDTSecretManagerState().getTokenState().put(identifier,
              renewDate);
          if (LOG.isDebugEnabled()) {
